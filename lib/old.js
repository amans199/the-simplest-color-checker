...
function applyColorRules(options) {
    if (!options || !options.targetSelector)
        return;
    const lightTextColor = options.lightTextColor || '#000';
    const darkTextColor = options.darkTextColor || '#fff';
    const elements = document.querySelectorAll(`${options.targetSelector}[data-color]`);
...
function checkColor(options) {
    var _a, _b;
    console.warn('`checkColor` is deprecated and will be removed in a future version. Use `applyColorRules` instead.');
    ...
    const mappedOptions = {
        ...
        lightTextColor: ((_a = options.ifLight) === null || _a === void 0 ? void 0 : _a.textColor) || '#000', // `ifLight.textColor` -> `lightTextColor`
        darkTextColor: ((_b = options.ifDark) === null || _b === void 0 ? void 0 : _b.textColor) || '#fff', // `ifDark.textColor` -> `darkTextColor`
    };
